...
class InventoryValuationXlsx(models.AbstractModel):
    ...
    def get_stock_data_all(self, products,location_objs, date_from=False, date_to=False):
        ...
        query2 = """
            ...
            where sm.date>%s and sm.date<=%s and sm.product_id in %s and sm.state='done' and ld.usage = 'internal' and ld.id in %s
        ...
        query3 = """
            ...
            LEFT JOIN product_template pt ON (product.product_tmpl_id = pt.id)
            LEFT JOIN uom_uom u on (u.id=sm.product_uom)
            where sm.date>%s and sm.date<=%s and sm.product_id in %s and sm.state='done' and ls.usage = 'internal' and ld.id in %s
            group by product.id,sm.state,sm.price_unit,locationid
            """
...
class InventoryValuationReport(models.TransientModel):
    ...
    def excel_report(self):
        ...
        report_xml_id = self.env.ref('aljaser_stock_movement_report_ext.report_xlsx_valuation_report')
        report_xml_id.report_file = 'Production Report'
        for field in datas['form'].keys():
            if isinstance(datas['form'][field], tuple):
                datas['form'][field] = datas['form'][field][0]
        return self.env.ref('aljaser_stock_movement_report_ext.report_xlsx_valuation_report').with_context(
