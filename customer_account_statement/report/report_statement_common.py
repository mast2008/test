...
class CustomerAccountStatementCommon(models.AbstractModel):
    """Abstract Report Statement for use in other models"""

    _name = "customer.account.statement.common"
    _description = "Customer Account Statement Common"
    ...
    def _get_account_display_lines(
        self, company_id, partner_ids, date_end, account_type
    ):
    ...
    def _show_buckets_sql_q2(self, date_end, minus_30, minus_60, minus_90, minus_120, minus_150, minus_180):
        return str(
            self._cr.mogrify(
                """
            ...
            END as b_90_120,
            CASE
                WHEN %(minus_150)s < date_maturity
                    AND date_maturity <= %(minus_120)s
                    AND currency_id is null
                THEN open_due
                WHEN %(minus_150)s < date_maturity
                    AND date_maturity <= %(minus_120)s
            ...
            END as b_120_150,
            CASE
                WHEN %(minus_180)s < date_maturity
                    AND date_maturity <= %(minus_150)s
                    AND currency_id is null
                THEN open_due
                WHEN %(minus_180)s < date_maturity
                    AND date_maturity <= %(minus_150)s
            ...
            END as b_150_180,
            CASE
                WHEN date_maturity <= %(minus_180)s
                    AND currency_id is null
                THEN open_due
                WHEN date_maturity <= %(minus_180)s
                    AND currency_id is not null
    ...
    def _show_buckets_sql_q3(self, company_id):
        return str(
            self._cr.mogrify(
                """
            SELECT Q2.partner_id, b_0_30, b_30_60, b_60_90, b_90_120,b_120_150,b_150_180,
                                b_over_180,
    ...
    def _show_buckets_sql_q4(self):
        return """
            SELECT partner_id, currency_id,
                sum(b_0_30) as b_0_30, sum(b_30_60) as b_30_60,
                sum(b_60_90) as b_60_90, sum(b_90_120) as b_90_120,
                sum(b_120_150) as b_120_150, sum(b_150_180) as b_150_180,
                sum(b_over_180) as b_over_180
            FROM Q3
            GROUP BY partner_id, currency_id
        """

    def _get_bucket_dates(self, date_end, aging_type):
        print("_get_bucket_dates3333333333",date_end, aging_type,type(date_end))
        return getattr(
            self, "_get_bucket_dates_%s" % aging_type, self._get_bucket_dates_days
        )(date_end)

    def _get_bucket_dates_days(self, date_end):
        print("_get_bucket_dates_days000000",date_end,type(date_end))
        return {
            "date_end": date_end,
            ...
            "minus_150": date_end - timedelta(days=150),
            "minus_180": date_end - timedelta(days=180),
        }

    def _get_bucket_dates_months(self, date_end):
        res = {}
        d = date_end
        for k in ("date_end", "minus_30", "minus_60", "minus_90", "minus_120", "minus_150", "minus_180"):
            res[k] = d
    ...
    def _get_account_show_buckets(
        ...
        print("_get_account_show_buckets",date_end, aging_type, type(date_end))
        ...
        self.env.cr.execute(
            ...
            WITH Q1 AS (%s),
                ...
                Q3 AS (%s),
                Q4 AS (%s)
            SELECT partner_id, currency_id, b_0_30, b_30_60, b_60_90,
                b_90_120, b_120_150, b_150_180, b_over_180,
                b_0_30+b_30_60+b_60_90+b_90_120+b_120_150+b_150_180+b_over_180
                AS balance
            FROM Q4
            GROUP BY partner_id, currency_id, b_0_30, b_30_60,
                b_60_90, b_90_120, b_120_150, b_150_180, b_over_180"""
            % (
                ...
                self._show_buckets_sql_q2(
                    ...
                    full_dates["minus_150"],
                    full_dates["minus_180"],
                ),
